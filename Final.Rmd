...
```{r}
#Adding logarithmic transformation for variables with huge outliers and which cover multiple orders of magnitude
data.avg.rating.1$Max_Players = log(data.avg.rating.1$Max_Players)
data.avg.rating.1$Avg_Time = log(data.avg.rating.1$Avg_Time+1)
data.avg.rating.1$Min_Time = log(data.avg.rating.1$Min_Time+1)
data.avg.rating.1$Max_Time = log(data.avg.rating.1$Max_Time+1)
data.avg.rating.1$Num_Votes = log(data.avg.rating.1$Num_Votes+1)
data.avg.rating.1$Num_Owned = log(data.avg.rating.1$Num_Owned+1)
data.avg.rating.1$Duration = log(data.avg.rating.1$Duration)
```
