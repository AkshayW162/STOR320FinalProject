...
#Q2
First, we removed the Greek_rating variable since our goal is to predict the average rating. Then we graphed the binary variables (mechanics and categories) to inspect which variables seem to have a relationship with avg_rating. We found out that ….[PATRICk]. After that, we performed a step-wise regression to obtain subsets of predictors that best explain the variability of average rating. The top ten models with the highest adjusted R square had similar sets of predictors; therefore we decided to continue with only 5 of them. We calculated the MAE of the models. [MAE.table] &[adjr.table][put the side by side if you can]
